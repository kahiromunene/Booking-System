  ...
  <div class="box">
    ...
    <div class="box-body">
	  ...
	  foreach($records as $rec) {
        ...
        <tr>
          <td><?php echo $idx++; ?></td>
          <td><a href="<?php echo WEB_ROOT; ?>views/?v=USER&ID=<?php echo $booking_id; ?>"><?php echo strtoupper($user_name); ?></a></td>
		  ...
		  <td><?php if($status == "PENDING") {?>
            <a href="javascript:approve('<?php echo $booking_id ?>');">Approve</a>&nbsp;/
			&nbsp;<a href="javascript:decline('<?php echo $booking_id ?>');">Denied</a>&nbsp;/
			&nbsp;<a href="javascript:deleteUser('<?php echo $booking_id ?>');">Delete</a>
            <?php } else { ?>
			<a href="javascript:deleteUser('<?php echo $booking_id ?>');">Delete</a>
			<?php }//else ?>
          </td>
...
function approve(booking_id) {
	if(confirm('Are you sure you wants to Approve it ?')) {
		window.location.href = '<?php echo WEB_ROOT; ?>api/process.php?cmd=regConfirm&action=approve&userId='+booking_id;
	}
}
function decline(booking_id) {
	if(confirm('Are you sure you wants to Decline the Booking ?')) {
		window.location.href = '<?php echo WEB_ROOT; ?>api/process.php?cmd=regConfirm&action=denide&userId='+booking_id;
	}
}
function deleteUser(booking_id) {
	if(confirm('Deleting user will also delete it\'s booking from calendar.\n\nAre you sure you want to priceed ?')) {
		window.location.href = '<?php echo WEB_ROOT; ?>api/process.php?cmd=delete&userId='+booking_id;
	}
}
